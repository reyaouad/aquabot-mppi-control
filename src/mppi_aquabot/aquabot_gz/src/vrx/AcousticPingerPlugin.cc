...
class AcousticPingerPlugin::Implementation
{
  ...
  public: transport::Node::Publisher rangePub;
  public: transport::Node::Publisher bearingPub;
  ...
  /// \brief The next messages to be published.
  public: msgs::Double rangeMsg;
  public: msgs::Double bearingMsg;
  ...
  sim::EntityComponentManager &_ecm, sim::EventManager &_eventMgr)
{
  ...
  // Setup the publisher.
  this->dataPtr->rangePub =
      this->dataPtr->node.Advertise<msgs::Double>(this->dataPtr->topic + "/range");
  this->dataPtr->bearingPub =
      this->dataPtr->node.Advertise<msgs::Double>(this->dataPtr->topic + "/bearing");
  ...
  sim::EntityComponentManager &_ecm)
{
  ...
  // Update messages
  this->dataPtr->rangeMsg.set_data(range);
  this->dataPtr->bearingMsg.set_data(bearing);

  // Publish a new measurement.
  this->dataPtr->bearingPub.Publish(this->dataPtr->bearingMsg);
  this->dataPtr->rangePub.Publish(this->dataPtr->rangeMsg);
}
